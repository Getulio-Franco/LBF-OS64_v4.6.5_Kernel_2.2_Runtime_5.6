...
void EnviarMensagemTeclado(char ascii) {
    ...
    if (destino_slot != -1 && destino_slot != my_app_slot) {
        ...
        // Se a struct AppWindowInfo oficial na SDK não expõe os campos do teclado virtual,
        // nós acessamos a estrutura mapeada dinamicamente via ponteiro de byte com base nos offsets,
        // ou reaproveitamos a estrutura de eventos enviando via payload de mensagem.
        // Abaixo, assumindo que os campos estão no final da estrutura de cada slot:
...
int main(int argc, char* argv[]) {
    ...
    GUI_InitApplication(&MyApp, my_app_slot, "Teclado Virtual OS v1.0", winWidth, winHeight);
    ...
    while(1) {
        ...
        // O teclado virtual nunca deve reter foco no loop principal
        bool euTenhoFoco = false; 
        if (MyApp.MainWindow) {
        ...
        sys_sleep(16); // Manter taxa de atualização responsiva estável de ~60 FPS para background
    }
