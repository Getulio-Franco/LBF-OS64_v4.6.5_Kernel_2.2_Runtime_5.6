...
void Coletar_Slots_Inativos(void) {
    ...
    if (!algum_app_ativo && lblStatus) {
        if (strcmp(lblStatus->Graphic.Control.Caption, "LBF-OS RUNTIME v5.6 [KERNEL 2.2]") != 0) {
            strcpy(lblStatus->Graphic.Control.Caption, "LBF-OS RUNTIME v5.6 [KERNEL 2.2]");
        }
    }
...
void Setup_Interface_Nativa(void) {
    ...
    lblStatus = gui_create_label((TWinControl*)frmTaskbar, "lblStatus", "LBF-OS RUNTIME v5.6 [KERNEL 2.2]");
    if (lblStatus) {
